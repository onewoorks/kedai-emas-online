...
const PointLog = (props) => {
    ...
    React.useEffect(() => {
        axios.get(`https://onewoorks-solutions.com/membership/public/person-and-point-card/${props.person_id}`)
            .then((response) => {
                axios.get(`https://onewoorks-solutions.com/membership/public/person/point-log/ariffin-kemaman/${response.data.person_id}`)
                    .then((response) => {
    ...
    const MobileView = () => {
        if (pointLog.length === 0) {
            return (
                <div >Tiada rekod transaksi setakat ini</div>
            )
        } else {
            return (
                pointLog.map((value, key) => {
                            ...
                            <div className="card-body p-2">
                                <div className="row">
                                    <div className="col-8 my-auto">
                                        <div style={{fontSize:'0.9em'}}>{value.perkara}</div>
                                    ...
                                    <div className="col-4 text-center my-auto">
                                        <div  style={{fontSize:'0.9em'}}>{value.type}</div>
                                        <div className="text-center log-number">{(value.mata_ganjaran * 0.01).toFixed(0)}</div>
                                    </div>
                                </div>
